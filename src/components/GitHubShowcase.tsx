...
export default function GitHubShowcase() {
  ...
  // Create simulated commitments grid for 32 columns x 7 rows (224 nodes)
  const commitGridData: CommitNode[][] = useMemo(() => {
    const days = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
    const grid: CommitNode[][] = Array.from({ length: 7 }, () => []);
    
    // Create random, but logically grouped high-frequency nodes representing standard AI work patterns
    let totalComputed = 0;
    for (let row = 0; row < 7; row++) {
      for (let col = 0; col < 32; col++) {
        // High frequency in mid columns (representing semester hackathons)
    ...
    return grid;
  }, [totalCommits]); // Re-calculate if recruiter requests random mock commits regeneration!
  ...
  return (
    <section id="github" className="py-24 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Heading */}
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 border border-slate-200 rounded-full text-slate-700 text-xs font-mono font-semibold">
            <Sparkles className="h-3 w-3 text-slate-500" />
            06 . GIT LOGS & REPOSITORIES
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900">
            Open Source Synthesis & Activity
          </h2>
          <p className="text-sm sm:text-base text-slate-500 max-w-2xl mx-auto">
            Live simulated GitHub engine displaying continuous development output, package breakdowns, and active code repositories.
          </p>
        </div>

        {/* GitHub Contribution Graph Section */}
        <div className="p-6 sm:p-8 rounded-3xl bg-slate-50 border border-slate-150 shadow-sm space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-mono">
                  {totalCommits} Contributions in the last 224 days
                </p>
              </div>
            </div>

            {/* Recruiter Game boost */}
            <button
              id="simulate-github-commits"
          ...
          <div className="flex justify-between items-center text-[10px] font-mono text-slate-400 pt-2 border-t border-slate-150/50">
            ...
            <div>
              {hoveredNode ? (
                ...
                <span>Hover over contribution cells of Dhruv</span>
              )}
            </div>

            <div className="flex items-center gap-1">
              <span>Less</span>
              <span className="h-2.5 w-2.5 rounded-xs bg-slate-100 border border-slate-200" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-12 items-start">
          {/* Most Used Languages */}
          <div className="lg:col-span-5 p-6 rounded-3xl bg-slate-50 border border-slate-150 shadow-sm space-y-6">
            ...
            <div className="space-y-4">
              {languages.map((lang) => (
                <div
                  id={`lang-line-${lang.name.toLowerCase().replace(/\s/g, "-")}`}
                  key={lang.name}
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            {popularRepos.map((repo, idx) => (
              <motion.div
                id={`git-repo-${repo.name}`}
                key={repo.name}
                whileHover={{ x: 5 }}
                transition={{ duration: 0.2 }}
                className="p-5 bg-white border border-slate-150 rounded-2xl hover:border-slate-350 shadow-xs hover:shadow-md transition-all duration-300"
              >
                ...
                <div className="flex items-center justify-between gap-4 mt-4 pt-3 border-t border-slate-50 text-[10px] font-mono text-slate-400">
                  <span>Language Framework: {repo.lang}</span>
                  <a
                    id={`git-repo-code-${repo.name}`}
